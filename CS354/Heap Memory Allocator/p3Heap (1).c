...
//
// Online sources:  include Web URLs and description of any information you find.
// 
// AI tools:        save the prompts and submit with project. 
////////////////////////////////////////////////////////////////////////////////
...
void* balloc(int size) {     
	...
	int hdr = sizeof(blockHeader);
        ...
        // Calculate allocation size and padding
        padding = (size + hdr) % 8;
        if (padding != 0){
                padding = 8 - padding;
        }
        ...
        // Loops through to find the best free block 
        while (curr->size_status != 1) {
                new_size = curr->size_status;
                if((new_size % 8) == 0 || (new_size % 8) ==  2){
                        // Checks if the block is greater than needed to allocate
                        if ((new_size / 8)*8 >= size_needed){
                                if (((new_size / 8) < psize / 8)){
                                        best = curr;
                                        psize = new_size;
                                }
                        }
                }
                new_size = (new_size/8)*8; // Get size of the header
                curr = (blockHeader*)((char*)curr + new_size); // Points to next header
        }

        // Set pointers to the best and next
...
int bfree(void *ptr) {    
	...
	if (ptr == NULL) {
        ...
        if ((void*)ptr < (void*)(heap_start + 1) || 
            (void*)ptr >= (void*)heap_start + alloc_size) {
                return -1;
        }
        ...
        int blockSize = (header->size_status / 8) * 8;
        
        // Validate block status (check if already free)
        if ((header->size_status & 1) == 0) {
                return -1;
        }

        // Clear allocated bit but keep previous block status
        header->size_status &= ~1;
        
        // Set up footer for the free block
        blockHeader *footer = (blockHeader*)((char*)header + blockSize - sizeof(blockHeader));
        footer->size_status = blockSize;

        // Update next block's previous-block status if it exists
        blockHeader *next = (blockHeader*)((char*)header + blockSize);
        if (next->size_status != 1) {  // If not end mark
                next->size_status &= ~2;  // Clear previous-allocated bit
        }

        // Coalesce free blocks
        blockHeader *current = heap_start;
        while (current->size_status != 1) {
                blockHeader *next_block = (blockHeader*)((char*)current + (current->size_status & ~7));
                
                // If both current and next blocks are free
                if (!(current->size_status & 1) && next_block->size_status != 1 && !(next_block->size_status & 1)) {
                        // Merge blocks
                        current->size_status = (current->size_status & ~7) + (next_block->size_status & ~7);
                        // Update footer
                        footer = (blockHeader*)((char*)current + (current->size_status & ~7) - sizeof(blockHeader));
                        footer->size_status = current->size_status & ~7;
                        continue;
                }
                current = next_block;
        }
